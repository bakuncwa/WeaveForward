...
class IsTUABWithSubscription(IsAuthenticated):
    """Permission to verify user is authenticated TUAB with active subscription using prediction service"""
...
class MatchRecommendationViewSet(viewsets.GenericViewSet, PaginatedResponseMixin):
    """
    ViewSet for handling match recommendations using prediction service.
    Displays routed donations assessed as compatible with TUAB's material requirements.
    
    Endpoints:
    - GET /match-recommendations/ - List pending recommendations (sorted by match confidence)
    ...
    def get_queryset(self):
        """Get base queryset filtered by TUAB, ranked by match confidence descending"""
    ...
    def list(self, request, *args, **kwargs):
        """
        List pending match recommendations sorted by match confidence.
        Uses prediction service for fiber metadata validation.
        
        Query parameters:
        - fiber_type: comma-separated list
        ...
        if request.query_params.get('confidence_min'):
            try:
                ...
                return Response(
                    {'detail': 'confidence_min must be 0-100'},
        ...
        if request.query_params.get('confidence_max'):
            try:
                ...
                return Response(
                    {'detail': 'confidence_max must be 0-100'},
                    status=status.HTTP_400_BAD_REQUEST
                )
        ...
        if request.query_params.get('date_after'):
            try:
                ...
                return Response(
                    {'detail': 'date_after must be ISO 8601'},
                    status=status.HTTP_400_BAD_REQUEST
                )
        ...
        if request.query_params.get('date_before'):
            try:
                ...
                return Response(
                    {'detail': 'date_before must be ISO 8601'},
                    status=status.HTTP_400_BAD_REQUEST
                )
    ...
    def _get_filtered_recommendations(self, tuab, filters):
        """Get pending recommendations with optional filters, ranked by match confidence"""
    ...
    def accept(self, request, pk=None):
        """Accept recommendation via prediction service context"""
        with transaction.atomic():
            try:
                ...
                return Response(
                    {'detail': 'Recommendation not found or no longer pending'},
            ...
            try:
                ...
                logger.info(f"TUAB {request.user.user_id} accepted recommendation {pk}")
                ...
                logger.error(f"Error accepting recommendation {pk}: {str(e)}")
                return Response(
                    {'detail': 'Failed to process acceptance'},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )
    
    @action(detail=True, methods=['post'])
    def reject(self, request, pk=None):
        """Reject recommendation with reason via prediction service context"""
        ...
        with transaction.atomic():
            ...
            try:
                ...
                logger.info(f"TUAB {request.user.user_id} rejected recommendation {pk}")
                ...
                logger.error(f"Error rejecting recommendation {pk}: {str(e)}")
                return Response(
